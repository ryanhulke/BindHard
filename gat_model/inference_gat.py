...
def main():
    ...
    prior = (
        AtomCountPrior.from_state_dict(ckpt["prior"])
        if isinstance(ckpt, dict) and "prior" in ckpt
        else AtomCountPrior.fit(dm.ds_train, n_bins=10)
    )

    out_dir = Path("inference") / ("gat_" + Path(cfg.ckpt).stem)
    ...
    for i, batch in enumerate(tqdm(dm.test_dataloader(), desc="sample")):
        ...
        torch.save(out, out_dir / f"sample_{i:06d}.pt")
