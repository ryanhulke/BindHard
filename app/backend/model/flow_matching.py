...
from model.common import AtomCountPrior, GaussianSmearing, compose_context, sample_categorical, center_by_protein
...
class LigandFlowMatching(torch.nn.Module):
    ...
    def sample(
        ...
        for step in range(self.t):
            ...
            probs_clean = torch.softmax(out["v0_logits"].float(), dim=-1)
    ...
    def loss(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        out = self.predict_outputs(batch, protein_pos, xt, vt_idx, t_graph)

        loss_pos = (out["flow_hat"].float() - flow_target.float()).pow(2).sum(dim=-1).mean()
        loss_type = F.cross_entropy(out["v0_logits"], v0_idx) * self.type_loss_scale
